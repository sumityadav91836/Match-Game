...
import ThumbnailImages from './Component/ThumbnailImages'
...
class App extends Component {
  ...
  onClickPlayAgain = () => {
    ...
    this.timerId = setInterval(this.changingTime, 1000)
  }
  ...
  onClickThumbnailBtn = uniqueId => {
    const {prevImg} = this.state
    const findImgUrl = imagesList.find(eachImg => eachImg.imageUrl === prevImg)
    if (findImgUrl.id === uniqueId) {
      this.setState(prevState => ({score: prevState.score + 1}))
      this.setState({
        prevImg: imagesList[Math.floor(Math.random() * 30)].imageUrl,
      })
    } else {
      this.setState({
  ...
  renderGameStartScreen = () => {
    const {prevImg, activeTabId} = this.state
    const filteredResult = this.getFilteredThumbnails()

    return (
      <>
        <img src={prevImg} alt="match" className="main-img" />
        ...
        <ul className="thumbnails-container">
          {filteredResult.map(eachImg => (
            <ThumbnailImages
              key={eachImg.id}
              imgDetails={eachImg}
              onClickThumbnailBtn={this.onClickThumbnailBtn}
            />
          ))}
        </ul>
      </>
  ...
  render() {
    const {timer, score, isGameInProcess} = this.state
